...
import { parseNL, nlFromServer, fallbackProject, NLChips, type NL } from '../screens/AiPage';
import { api } from '../api';
...
export function QuickAdd() {
  ...
  const wsFallback = fallbackProject(s);
  const qaProjEff = (projOv ? s.proj(projOv) : null) || (qa && qa.proj) || wsFallback;

  // Effective (edited) values — overrides win over the parsed values.
  const effA = aOv !== undefined ? aOv : (qa?.assignee ?? null);
  const effDue = dueOv !== undefined ? dueOv : (qa?.due ?? null);
  const effPr = prOv !== undefined ? prOv : (qa?.pr ?? 'med');

  const canWriteWs = (ws: string) => { const r = s.myRoles[ws]; return r !== 'GUEST' && r !== 'EXEC_VIEWER'; };
  const noWritable = !s.projects.some((p) => p.ws === s.ws && canWriteWs(p.ws));
  ...
  const openNewProject = () => {
    s.set({ quickAdd: false, qaText: '', onb: { step: 3, newProj: true, wsName: '', inv: '', invites: [], desc: '', wbs: null, mode: 'tpl', busy: false } });
    resetOv();
  };

  const create = () => {
    if (!qa || noWritable) return;
    if (!qaProjEff) { s.pushToast('No project in this workspace yet', 'bad'); return; }
    s.addTask(qa.title, qaProjEff.id, null, effDue ?? undefined, { a: effA || null, pr: effPr || 'med' });
    close();
    s.pushToast('Task created in ' + qaProjEff.name);
  };
  const edit = () => {
    if (!qa || noWritable) return;
    if (!qaProjEff) { s.pushToast('No project in this workspace yet', 'bad'); return; }
    const id = s.addTask(qa.title, qaProjEff.id, null, effDue ?? undefined, { a: effA || null, pr: effPr || 'med' });
    s.set({ quickAdd: false, qaText: '', screen: 'project', projectId: qaProjEff.id });
    resetOv();
    s.openTask(id);
  };
  // Short inputs: create the raw text as a simple task due today.
  const createSimple = () => {
    const txt = s.qaText.trim();
    if (!txt || noWritable) return;
    const target = (projOv ? s.proj(projOv) : null) || wsFallback;
    if (!target) { s.pushToast('No project in this workspace yet', 'bad'); return; }
    s.addTask(txt, target.id, null, TODAY);
    close();
  ...
  return (
    <div onMouseDown={close} style={{ position: 'fixed', inset: 0, zIndex: 85 }}>
      <div onMouseDown={(e) => e.stopPropagation()} style={{ position: 'absolute', top: 56, right: 'clamp(12px,calc(100vw - 520px),170px)', width: 'min(480px,90vw)', background: 'var(--glass)', backdropFilter: 'blur(18px)', border: '1px solid var(--line)', borderRadius: 16, boxShadow: 'var(--sh3)', padding: 14, animation: 'vpop .16s ease' }}>
        ...
        <textarea ref={taRef} value={s.qaText} onChange={(e) => s.set({ qaText: e.target.value })} onKeyDown={onKey} placeholder="Try: Create task 'Review PO automation', assign Budi, due Friday, high priority, project Connected Manufacturing" style={{ width: '100%', minHeight: 58, resize: 'none', background: 'var(--inputBg)', border: '1px solid var(--line)', borderRadius: 10, padding: '9px 11px', fontSize: 12.5, color: 'var(--txt)', lineHeight: 1.5 }} />
        {qaPrevOn && qa && (
          <div style={{ marginTop: 10, border: '1px solid var(--acc2)', background: 'var(--accS)', borderRadius: 12, padding: 11, animation: 'vup .18s ease' }}>
            ...
            <div style={{ display: 'flex', gap: 7, marginTop: 11, alignItems: 'center' }}>
              ...
              <span onClick={noWritable ? undefined : edit} style={{ fontSize: 11.5, fontWeight: 600, color: noWritable ? 'var(--txt3)' : 'var(--txt2)', border: '1px solid var(--line)', borderRadius: 8, padding: '6px 12px', cursor: noWritable ? 'not-allowed' : 'pointer', background: 'var(--card)' }}>Edit fields</span>
            </div>
          </div>
        )}
        {!qaPrevOn && s.qaText.trim() && !noWritable && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 9 }}>
            ...
            <span style={{ fontSize: 10.5, color: 'var(--txt3)' }}>due today in {((projOv ? s.proj(projOv) : null) || wsFallback)?.name || '—'} · or press Ctrl/⌘+Enter</span>
          </div>
        )}
        {noWritable && (
          <div style={{ marginTop: 9, fontSize: 11, color: 'var(--txt2)', border: '1px dashed var(--line)', borderRadius: 10, padding: '8px 11px' }}>
            Buat project dulu — belum ada project yang bisa ditulis di workspace ini. <span onClick={openNewProject} style={{ color: 'var(--accT)', cursor: 'pointer', fontWeight: 700 }}>Buat project →</span>
          </div>
        )}
        {!qaPrevOn && !s.qaText.trim() && !noWritable && (
          <div style={{ marginTop: 8, fontSize: 10.5, color: 'var(--txt3)' }}>Coba juga: <span onClick={() => s.set({ qaText: "Buat tugas 'Kalibrasi mesin CNC', tugaskan ke Dewi, tenggat Jumat, prioritas tinggi" })} style={{ color: 'var(--accT)', cursor: 'pointer', fontWeight: 600 }}>"Buat tugas 'Kalibrasi mesin CNC', tugaskan ke Dewi, tenggat Jumat, prioritas tinggi"</span></div>
        )}
      </div>
    </div>
  );
